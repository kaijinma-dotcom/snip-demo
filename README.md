...
The `bundle` branch is a self-contained deployable generated by `scripts/build-bundle.mjs`.
It combines `backend/server.js`, `cli/cli.js`, and the Angular build output into a single
directory that can be deployed with Docker or Railway.

```sh
# Rebuild and publish (run from the superproject root)
node scripts/build-bundle.mjs --push
```

The script is a **safe no-op** when nothing has changed since the last build.
